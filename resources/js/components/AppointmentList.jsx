    ...
    if (appointments.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-8 text-center">
                <svg
                    className="w-12 h-12 text-[#E0E0E0] mb-3"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                >
                ...
                <p className="text-sm text-[#6D6D6D]">No appointments yet</p>
            </div>
        );
    }
        ...
        <div className="appointment-list">
            {groupedAppointments.map(({ date, appointments: dateAppointments }) => (
                <div key={date} className="mb-4">
                    {/* Date Header */}
                    <div className="sticky top-0 bg-[#F5F5F5] px-3 py-2 text-xs font-semibold text-[#6D6D6D] border-b border-[#E0E0E0]">
                        {formatDate(date)}
                    </div>
                        ...
                        {dateAppointments.map((apt) => (
                            ...
                            >
                                ...
                                <div className="flex items-center text-xs text-[#6D6D6D] mb-2">
                                    ...
                                    {apt.slot ? (
                                        <span>
                                            {formatTime(apt.slot.start_time)} - {formatTime(apt.slot.end_time)}
                                        </span>
                                    ) : (
